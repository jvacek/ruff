...
use crate::types::{KnownClass, NormalizedVisitor, SubclassOfType};
...
impl<'db> TypedDictType<'db> {
    ...
    pub(super) fn to_meta_type(self, db: &'db dyn Db) -> Type<'db> {
        match self {
            TypedDictType::Synthesized(_) => KnownClass::TypedDict.to_instance(db),
...
pub struct SynthesizedTypedDictType<'db> {
    // The dictionary literal passed to the `TypedDict` constructor is inferred as an
    // intermediary `SynthesizedTypedDictType`, with a name of `None`.
